...
if(isset($_POST['info_form']))
{
    ...
    session_start();
    //checks if phone number matches with any other User other than the auto incremented ID User has. `id`!=?
    $u_exist = select("SELECT * FROM `user_cred` WHERE `phonenum`=? AND `id`!=? LIMIT 1",
    [$data['phonenum'],$_SESSION['uId']],"ss");

    if(mysqli_num_rows($u_exist)!=0)
    {
      echo 'phone_already'; //? is ternery operator
      exit;
    }
...
if(isset($_POST['profile_form']))
{
    ...
    //upload new image of user to server
    ...
    //fetching and deleting old image of user
